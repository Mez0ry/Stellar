...
public:
  ...
  static void RegisterService(Args&&... args) {
    ...
    if(!HasService<T>()){
      services_map.insert(std::make_pair(context.GetHashCode<T>(), Base::CreateRef<T,Args ...>( std::forward<Args>(args)... )));
    }
  }
  ...
  template<typename T>
  [[nodiscard]] static Base::Ref<T> GetService(){
    auto& context = instance();
    auto& services_map = context.m_ServicesMap;
...
private:
  std::unordered_map<hash_code, Base::Ref<void>> m_ServicesMap;
};
